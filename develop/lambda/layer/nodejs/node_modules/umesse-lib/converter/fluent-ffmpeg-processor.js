        ...
        case "narrations":
          ...
          for (let v of value) {
            ...
            if (v.contentsId.match(`^${unisCustomerCd}-r-[0-9a-z]{8}$`)) {
              // レコーディング音源の場合
              target = `users/${unisCustomerCd}/${constants.resourceCategory.RECORDING}/${v.contentsId}.mp3`;
              bucket = constants.s3Bucket().users;
            } else if (v.contentsId.match(`^${unisCustomerCd}-t-[0-9a-z]{8}$`)) {
              // TTS音源の場合
              target = `users/${unisCustomerCd}/${constants.resourceCategory.TTS}/${v.contentsId}.mp3`;
              bucket = constants.s3Bucket().users;
            }
