
'use strict';

const fs = require('fs')
const path = require('path');
const { debuglog, constants } = require('../constants')
const { execSync } = require("child_process");

module.exports = function (proto) {
  let workDir;
  let windowsWorkDir;
  const silentFilePath = `${process.cwd()}\\tmp\\umesse\\silent.mp3`;

  proto.createReadStream = function (filename) {
    let basename = path.basename(filename);
    return fs.createReadStream(`${windowsWorkDir}/${basename}`);
  }

  proto.getWorkDir = function (unisCustomerCd, cmId) {
    return path.posix.join(this.basePath, unisCustomerCd, 'mix', cmId);
  }

  proto.getContentsDir = function (unisCustomerCd, cmId) {
    return this.getWindowsWorkDir(unisCustomerCd, cmId);
  }

  proto.getWindowsWorkDir = function (unisCustomerCd, cmId) {
    return path.win32.join(process.cwd(), convertPathPosixToWindows(this.basePath), unisCustomerCd, 'mix', cmId)
  }

  // windows exec.
  proto.winExecSync = function (command) {
    fs.writeFileSync(
      `${windowsWorkDir}/ffmpeg-command`,
      `#!/bin/bash\n${command}`
    );
    command = `docker run --name ffmpeg-runner --rm \
          -v ${process.cwd()}\\..\\layer\\bin:/usr/local/bin \
          -v ${windowsWorkDir}:${workDir} \
          -v ${silentFilePath}:/tmp/umesse/silent.mp3 \
          centos:latest \
          sh ${workDir}/ffmpeg-command`;
    debuglog(command);
    return execSync(command);
  }

  proto.generateCm = async function (unisCustomerCd, cmId, materials, output) {
    output = convertPathWindowsToPosix(output);
    workDir = this.getWorkDir(unisCustomerCd, cmId);
    windowsWorkDir = this.getWindowsWorkDir(unisCustomerCd, cmId);

    const initDirCommand = `mkdir ${windowsWorkDir} > NUL 2>&1 && \
          if ERRORLEVEL 1 cmd /c exit 0 && \
          rd /q ${windowsWorkDir}\\*`;
    execSync(initDirCommand);

    // S3から無音ファイルを取得
    let filePath = path.win32.join(process.cwd(), convertPathPosixToWindows(this.basePath));
    if (!(await this.getContents(constants.s3Bucket().contents, "silent.mp3", filePath, "silent.mp3")))
      throw "getObject failed";

    let command = await this.buildGenerateCM(unisCustomerCd, cmId, materials, output);
    this.winExecSync(command)
  }

  proto.getDuration = function (input) {
    input = convertPathWindowsToPosix(input);
    // CMの秒数を取得
    let command = this.buildGetDuration(input);
    return this.winExecSync(command).toString().replace(/\n/g, "");
  }

  function convertPathWindowsToPosix(path) {
    return path.replace(/\\/g, "/");
  }
  function convertPathPosixToWindows(path) {
    return path.replace(/\//g, "\\");
  }
}