    ...
    for (let [key, value] of Object.entries(materials).sort()) {
        ...
        switch (key) {
            ...
            case "startChime":
                ...
                if (!(await this.getContents(constants.s3Bucket().contents, `chime/${value.contentsId}.mp3`, workDir, `${fileName}.mp3`)))
                    throw "getObject failed";
                paths += `-i ${workDir}/${fileName}.mp3 `;
            ...
            case "bgm":
                fileName = "bgm";
                if (!(await this.getContents(constants.s3Bucket().contents, `bgm/${value.contentsId}.mp3`, workDir, `${fileName}.mp3`)))
                    throw "getObject failed";
                paths += `-i ${workDir}/${fileName}.mp3 `;
