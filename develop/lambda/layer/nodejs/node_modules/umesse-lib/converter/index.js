...
UMesseConverter.prototype.getContentsDir = function (unisCustomerCd, cmId) {
    return this.getWorkDir(unisCustomerCd, cmId);
}
UMesseConverter.prototype.buildGenerateCM = async function (unisCustomerCd, cmId, materials, output) {
    const contentsDir = this.getContentsDir(unisCustomerCd, cmId);
        ...
        switch (key) {
            case "narrations":
                ...
                for (let v of value) {
                    ...
                    if (!(await this.getContents(bucket, target, contentsDir, `${fileName}.mp3`)))
            ...
            case "startChime":
                ...
                if (!(await this.getContents(constants.s3Bucket().contents, `chime/${value.contentsId}.mp3`, contentsDir, `${fileName}.mp3`)))
                    throw "getObject failed";
                paths += `-i ${workDir}/${fileName}.mp3 `;
            ...
            case "bgm":
                fileName = "bgm";
                if (!(await this.getContents(constants.s3Bucket().contents, `bgm/${value.contentsId}.mp3`, contentsDir, `${fileName}.mp3`)))
                    throw "getObject failed";
                paths += `-i ${workDir}/${fileName}.mp3 `;
