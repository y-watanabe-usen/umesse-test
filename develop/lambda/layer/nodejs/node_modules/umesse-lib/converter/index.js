...
var UMesseConverter = function (s3Manager, unisCustomerCd, cmId) {
    const useFluentFFmpeg = false;
    this.s3Manager = s3Manager;
    this.ffmpeg = `ffmpeg -hide_banner`;
    this.basePath = '/tmp/umesse';

    if (useFluentFFmpeg == true)
        return require('./third-party')(UMesseConverter.prototype);
    ...
    if (this.isLocalWindows) {
        this.windowsWorkDir = `${process.cwd()}\\tmp\\umesse\\${unisCustomerCd}\\mix\\${cmId}`; // windows only
        // win用の関数上書き.
...
UMesseConverter.prototype.buildGenerateCM = async function (unisCustomerCd, cmId, materials, output) {
    ...
    const workDir2 = `${process.cwd()}\\tmp\\umesse\\${unisCustomerCd}\\mix\\${cmId}`;
    const has = {
        ...
        switch (key) {
            case "narrations":
                ...
                for (let v of value) {
                    ...
                    if (!(await this.getContents(bucket, target, workDir2, `${fileName}.mp3`)))
            ...
            case "startChime":
                ...
                if (!(await this.getContents(constants.s3Bucket().contents, `chime/${value.contentsId}.mp3`, workDir2, `${fileName}.mp3`)))
                    throw "getObject failed";
                paths += `-i ${workDir}/${fileName}.mp3 `;
            ...
            case "bgm":
                fileName = "bgm";
                if (!(await this.getContents(constants.s3Bucket().contents, `bgm/${value.contentsId}.mp3`, workDir2, `${fileName}.mp3`)))
                    throw "getObject failed";
                paths += `-i ${workDir}/${fileName}.mp3 `;
