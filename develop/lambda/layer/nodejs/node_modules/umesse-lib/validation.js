...
exports.checkParams = (params, addRequired) => {
  ...
  if (params.title) {
    if (params.title.match(regex)) {
      ret.push(`${ERROR_CODE.E0001051} (E0001051)`);
    }

    // sjis機種依存文字（センター都合）
    if (!isShiftJISValidator(params.title)) {
      ret.push(`${ERROR_CODE.E0001051} (E0001051)`);
    }
  }
  if (params.description) {
    if (params.description.match(regex)) {
      ret.push(`${ERROR_CODE.E0001061} (E0001061)`);
    }

    // sjis機種依存文字（センター都合）
    if (!isShiftJISValidator(params.description)) {
      ret.push(`${ERROR_CODE.E0001061} (E0001061)`);
    }
  }
...
function isShiftJISValidator(str) {
  let isValid = true;
  for (let c of str) {
    if (!c || Buffer.from(c).length === 1) continue;
    const v = Buffer.from(iconv.encode(Buffer.from(c), "windows-31j"));
    if (v.byteLength === 1) {
      isValid = false;
      continue;
    }
    for (let i = 0; i < v.byteLength; i++) {
      if (v.slice(i, i + 1) == Buffer.from([0x63])) {
        isValid = false;
        return;
      }
    }
